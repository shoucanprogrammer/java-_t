      ...
      //创建广播电台，放入map
        ...
        while (allAreas.size() != 0){//如果allAreas不为0，则表示还没有覆盖到所有地区
            ...
            for (String key : broadcasts.keySet()){
                ...
                //求出tempSet 和 allAreas集合的交集
                tempSet.retainAll(allAreas);
                //如果当前这个集合包含的未覆盖地区的数量，比maxKey指向的集合未覆盖的地区还多
                //就需要重置maxKey
                HashSet<String> max = null;
                //tempSet。size() > briadcasts.get(maxKey).size())体现出贪心算法的特点。每次都选择最优的
                if (maxKey != null) {
            ...
            if (maxKey != null){
                selects.add(maxKey);
                //将maxKey指向广播电台覆盖的地区，从allAreas去掉
